...
const Page = async ({
  ...
  if (agencyDetails.connectAccountId && agencyDetails.connectAccountSecret) {
    const instance = new Razorpay({
      key_id: agencyDetails.connectAccountId || "",
      key_secret: agencyDetails.connectAccountSecret || "",
    });

    const payments = await instance.payments.all();

    net =
      payments.items.reduce((total, payment) => {
    ...
    // })
    // const response = await stripe.accounts.retrieve("ajfdlkad");
  ...
  return (
    <div className="relative h-full">
      {!agencyDetails.connectAccountId && (
        <div className="absolute h-[125%] -top-10 -left-10 right-0 bottom-0 z-30 flex items-center justify-center backdrop-blur-md bg-background/50">
          <Card>
            <CardHeader>
        ...
        <div className="flex gap-4 flex-col xl:!flex-row">
          <Card className="flex-1 relative">
            ...
            <CardContent className="text-sm text-muted-foreground">
              Total revenue generated as reflected in your razorpay dashboard.
            </CardContent>
